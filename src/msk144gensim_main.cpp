...
public:
    ...
    MsgItem& msgItem() { return msg_item; }
...
static void usage(void)
{
    ...
    buf << "\nmsk144gensim - msk144 generator. Produces infinite stdout audio "
           "stream - 16 bits signed, 12000 samples per second, mono.\n\n";
    ...
    buf << "\t[--use-throttle= emulate real time output using sleep (default: "
           "false)]\n";
    buf << "\t[--show-only Print only text and bits to be send. (default: "
           "false)]\n";
    buf << "\t[--help this text]\n";
    ...
    while(true)
    {
        ...
        if(c == 0)
        {
            ...
            case 11:
                ...
                if(ctx.num_messages > NumMessagesMax)
                {
                    ctx.num_messages = NumMessagesMax;
                }
                if(ctx.num_messages < 0)
                {
                    ctx.num_messages = 0;
                }
                break;
